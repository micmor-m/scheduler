      ...
      const daysArray = state.days.map((day) => {
        for (let appointment of day.appointments) {
          if (action.id === appointment) {
            //if currently interview is null but I have an action interview (mean a new interview)
            if (action.interview && !state.appointments[action.id].interview) {
              return { ...day, spots: day.spots - 1 };
            } else if (
              //if currently interview is booked but I have NO action interview (mean a cancel the interview)
              !action.interview &&
